...
AI_PROVIDER=ollama
AI_API_KEY=local
AI_MODEL=fredrezones55/Qwen3.5-Uncensored-HauhauCS-Aggressive:9b
...
Use `AI_MODEL` to select the shared Ollama model for general AI features. Keep `AI_MODEL_REPLY` pointed at the same model when you want one model resident on the Ollama host.
