...
AI provider. Warmbly AI targets the shared Ollama OpenAI-compatible endpoint. Set these on the backend and the consumer:

```
AI_PROVIDER=ollama
AI_API_KEY=local
AI_MODEL=                # optional Ollama model override
AI_MODEL_REPLY=          # optional reply-classifier model override
AI_BASE_URL=https://ollama.pattrndata.io/v1
AI_FREE=true             # free/un-metered model (skips credits)
...
Use `AI_MODEL` to select the shared Ollama model for general AI features, and `AI_MODEL_REPLY` only if the reply classifier needs a different Ollama model.
