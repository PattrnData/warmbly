...
The AI features (the dashboard assistant, reply drafts, contact research, and automation AI nodes) target the shared Ollama OpenAI-compatible endpoint by default in local dev. Override `AI_MODEL` only when you want another Ollama model.

```bash
make backend
make run AI_MODEL=qwen2.5
```

The default dev env sets `AI_PROVIDER=ollama`, `AI_BASE_URL=https://ollama.pattrndata.io/v1`, `AI_API_KEY=local`, `AI_MODEL_REPLY=fredrezones55/Qwen3.5-Uncensored-HauhauCS-Aggressive:9b`, and `AI_FREE=true`, so AI credits are not charged and the assistant shows a "free model" notice. Use one shared Ollama model for the general assistant and reply classifier when you want to keep only one model resident on the host.
