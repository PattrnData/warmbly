...
The default dev env sets `AI_PROVIDER=ollama`, `AI_BASE_URL=https://ollama.pattrndata.io/v1`, `AI_API_KEY=local`, and `AI_FREE=true`, so AI credits are not charged and the assistant shows a "free model" notice. Use a tool-capable model such as `llama3.1` or `qwen2.5`, not a 1 to 3B model.
